...
void init(void)
{
	...
	unsigned char *data = stbi_load("../../src/resources/textures/wall2.jpg", &width, &height, &nrChannels, 0);
...
void display()
{
	...
	// modify model matrix if rotating
	if (rotater == true) {
		model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(1.0f, 1.0f, 0.0f));
	}
	if (grower == true) {
		float scaleAmount = abs(sin(glfwGetTime() * 3.0f));
		model = glm::scale(model, glm::vec3(scaleAmount * 2.5, scaleAmount * 2.5, scaleAmount * 2.5));
	}
	if (mover == true) {
		model = glm::translate(model, glm::vec3( 0.75f * sin((float)glfwGetTime()), -0.75f * sin((float)glfwGetTime()), 0.0f ));
	}
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Assignment 02: Q1 - Ben Abrams, Zack Hardee, Griffin Megeff", NULL, NULL);
	if (window == NULL)
	{
