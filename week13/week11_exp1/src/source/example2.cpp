...
void init(void)
{
	...
	unsigned char *data = stbi_load("../../src/resources/textures/dog.jpg", &width, &height, &nrChannels, 0);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	//load image, create texture and generate mipmaps
	data = stbi_load("../../src/resources/textures/box.png", &width, &height, &nrChannels, 0);
	generateTexture2D(data, width, height, nrChannels);
	...
	glUniform1f(glGetUniformLocation(shaderProgram, "shade"), shade);
}
...
void display()
{
	...
	if (rotflag == 3) {
		model = glm::rotate(model, -1 * (float)glfwGetTime(), glm::vec3(0.0f, 1.0f, 0.0f));
	}
