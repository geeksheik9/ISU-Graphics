...
void init(void)
{
	...
	lightingShader = loadShader("../../src/shader/2.2.3n2.basic_lighting.vert", "../../src/shader/2.2.3n2.basic_lighting.frag");
	lampShader = loadShader("../../src/shader/2.2.3n2.lamp.vert", "../../src/shader/2.2.3n2.lamp.frag");
...
void display()
{
	...
	if (material == 1) {
		unsigned int materialAmbientLoc = glGetUniformLocation(lightingShader, "material.ambient");
		glUniform3f(materialAmbientLoc, 0.0215f, 0.1745f, 0.0215f);

		unsigned int materialDiffuseLoc = glGetUniformLocation(lightingShader, "material.diffuse");
		glUniform3f(materialDiffuseLoc, 0.07568f, 0.61424f, 0.07568f);

		//specular lighting doesn't have full effect on this object's material
		unsigned int materialSpecularLoc = glGetUniformLocation(lightingShader, "material.specular");
		glUniform3f(materialSpecularLoc, 0.633f, 0.727811f, 0.633f);
	...
	if (material == 2) {
		unsigned int materialAmbientLoc = glGetUniformLocation(lightingShader, "material.ambient");
		glUniform3f(materialAmbientLoc, 0.05375f, 0.05f, 0.1575f);

		unsigned int materialDiffuseLoc = glGetUniformLocation(lightingShader, "material.diffuse");
		glUniform3f(materialDiffuseLoc, 0.18275f, 0.17f, 0.22525f);

		//specular lighting doesn't have full effect on this object's material
		unsigned int materialSpecularLoc = glGetUniformLocation(lightingShader, "material.specular");
		glUniform3f(materialSpecularLoc, 0.332741f, 0.328634f, 0.346435f);
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Materials: Example 1", NULL, NULL);
	if (window == NULL)
	{
